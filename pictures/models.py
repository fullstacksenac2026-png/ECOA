...
class Comment(models.Model):
    ...
    def __str__(self):
        return f"Comentário de {self.user.username} - {self.content[:20]}"

class Like(models.Model):
    picture = models.ForeignKey(Picture, on_delete=models.CASCADE, related_name='likes', null=True, blank=True)
    ...
    def __str__(self):
        interaction = "Like" if self.is_like else "Dislike"
        return f"{interaction} de {self.user.username}"
